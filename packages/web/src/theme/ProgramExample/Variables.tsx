...
export function Variables(): JSX.Element {
  ...
  const link = <Link to="/spec/program/context/variables">
  ...
  if (!(context && "variables" in context)) {
    return <Admonition type="warning" title="No variables at instruction">
...
function Variable(props: VariableProps): JSX.Element {
  ...
  const details = (["type", "pointer"] as const)
    ...
    .map((detail) => <div key={detail}>
      <h3>{`${detail.slice(0,1).toUpperCase()}${detail.slice(1)}`}</h3>

      <ShikiCodeBlock
        language="javascript"
        code={
          JSON.stringify(variable[detail], undefined, 2)
        }
      />
    </div>)
  return <>
