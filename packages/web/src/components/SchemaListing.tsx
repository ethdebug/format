...
export default function SchemaListing(props: SchemaListingProps): JSX.Element {
  const { id: qualifiedId, pointer, schema, yaml } = describeSchema(props);
  ...
  const reference = id && pointer ? `${id}${pointer}` : id ? id : undefined;
    ...
    <Tabs groupId="schema-language">
      <TabItem value="yaml" label="YAML">
        <CodeBlock
          ...
          language="yaml"
          showLineNumbers
          title={reference ? reference : "schema.yaml"}
        >
          {yaml}
        </CodeBlock>
      </TabItem>
      <TabItem value="json" label="JSON">
        <CodeBlock
          ...
          language="json"
          showLineNumbers
          title={reference ? `${reference}` : "schema.json"}
        >
