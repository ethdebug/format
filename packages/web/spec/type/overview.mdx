...
:::tip[Conceptual introduction]
For the mental model behind type representations, see
[Concepts: Types](/docs/concepts/types).
:::
...
- The [Base schema](/spec/type/base) defines the minimum overall structure of
...
<Tabs>
  {[
    ...
    return (
      <TabItem key={index} value={index} label={id.split("/").pop()}>
        <CodeBlock language="json" title={`Example ${id} type`}>
