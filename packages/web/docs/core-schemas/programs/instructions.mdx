...
<SchemaExample
  ...
  href="/spec/program/instruction"
  title="Instruction with operation"
>
  {`{
  "offset": 0,
  "operation": {
...
- **`mnemonic`** (required): the opcode name (e.g., `"PUSH1"`,
  `"SSTORE"`, `"ADD"`)
- **`arguments`** (optional): immediate values passed to the opcode
  (relevant for `PUSH` instructions)
