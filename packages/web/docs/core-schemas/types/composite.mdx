...
Function types must specify either `"internal": true` or
`"external": true`. Internal functions are called within the
language's own semantics; external functions are called via EVM
contract call semantics (e.g., the Solidity ABI).
