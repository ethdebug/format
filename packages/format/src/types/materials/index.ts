...
export namespace Materials {
  ...
  export const isReference = <O extends { id: Id }>(
    ...
    typeof value === "object" && !!value && "id" in value && isId(value.id);
  ...
  export const isCompilation = (value: unknown): value is Compilation =>
    ...
    !!value.compiler &&
    "name" in value.compiler &&
    typeof value.compiler.name === "string" &&
  ...
  export const isSourceRange = (value: unknown): value is SourceRange =>
    ...
    !!value &&
    "source" in value &&
    isReference<Source>(value.source) &&
    (!("range" in value) ||
      (typeof value.range === "object" &&
        !!value.range &&
        "offset" in value.range &&
        Data.isValue(value.range.offset) &&
        "length" in value.range &&
        Data.isValue(value.range.length))) &&
